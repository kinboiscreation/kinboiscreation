...
description: >
  ...
  identical and sacred — the five-tone warm-stone palette, Space Grotesk 400 + Space Grotesk, the
...
typography:
  ...
  body:        { fontFamily: "Space Grotesk", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "Space Grotesk", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "Space Grotesk", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "Space Grotesk", px: 14, weight: 500, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "Space Grotesk", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "Space Grotesk", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
  # — display / hero ramp (Cormorant Garamond 400, sentence case — matches the live site's h1/h2/h3 CSS) —
...
- **Fonts** — already set to **Space Grotesk** (display) / **Cormorant Garamond** (body); ignore any preset font name lingering in prose.
...
The voice is a literary pairing: **Space Grotesk** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **Space Grotesk**
...
- **Space Grotesk 400** (ink, sentence case) for display; **Space Grotesk** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (Space Grotesk body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **Space Grotesk labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs Space Grotesk sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an Space Grotesk subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **Space Grotesk + Space Grotesk via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
